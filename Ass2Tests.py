...
def runTest(file, inText):
    if file == "order":
        res = subprocess.run("python " + file + ".py", input=inText,
                             capture_output=True, text=True, shell=True)
        return res.stdout

    if file == "pizzaReceipt":
        res = subprocess.run(
            "python -c \"from pizzaReceipt import *; generateReceipt([" + inText + "])\"", capture_output=True, text=True, shell=True)
        return res.stdout
